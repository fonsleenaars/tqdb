...
class CharacterParser(TQDBParser):
    ...
    def parse(self, dbr, dbr_file, result):
        ...
        result["properties"]["offensivePhysical"] = parsers.ParametersOffensiveParser.format(
            parsers.ParametersOffensiveParser.ABSOLUTE, "offensivePhysical", dmg_min, dmg_max
        )
...
class MonsterParser(TQDBParser):
    ...
    def parse(self, dbr, dbr_file, result):
        ...
        for index in range(3):
            ...
            if tag in CHESTS and CHESTS[tag][index]:
                ...
                level = result["level"][index] if len(result["level"]) > index else result["level"][-1]
                ...
                chests[index] = dict((k, float("{0:.4f}".format(v))) for k, v in loot["loot_table"].items())
    ...
    def parse_difficulty(self, dbr, dbr_file):
        ...
        for equipment in self.EQUIPMENT_SLOTS:
            ...
            summed = sum(v for k, v in dbr.items() if k.startswith(equip_key))
            ...
            for i in range(1, 7):
                ...
                if "tag" in loot:
                    ...
                    self.add_items(result, {loot["tag"]: chance * equip_chance})
                elif "loot_table" in loot:
                    # Add all the items (and multiply their chances)
                    items = dict((k, v * chance * equip_chance) for k, v in loot["loot_table"].items())
                    self.add_items(result, items)

        # Convert all item chances to 4 point precision:
        result = dict((k, float("{0:.4f}".format(v))) for k, v in result.items())
...
class MonsterSkillManager(TQDBParser):
    ...
    IGNORE_SKILLS = list(
        ...
        "data\\database\\records\\xpack\\skills\\scroll skills\\pets\\" "all_breakingwheel_bladeattack2.dbr",
        "data\\database\\records\\xpack\\skills\\scroll skills\\pets\\" "all_breakingwheel_bladeattack3.dbr",
    ]
    ...
    def parse(self, dbr, dbr_file, result):
        ...
        for i in range(1, 18):
            ...
            if nameTag not in dbr or levelTag not in dbr or str(dbr[nameTag]).lower() in self.IGNORE_SKILLS:
                continue
